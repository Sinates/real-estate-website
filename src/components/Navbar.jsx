  ...
  document.body.style.overflow = 'auto'};
    ...
    <div className='absolute top-0 left-0 w-full z-10'>
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md :px-20 lg:px-32 bg-transparent'>
        ...
        <img  onClick={() => setShowMobileMenu(true)}  src="{assets.icon.png}" className='md:hidden w-7' alt="" />
      ...
      <div className={`md:hidden ${ShowMobileMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        ...
        <div className='flex justify-end p-6 cursor-pointer'> 
           <img onClick={() => setShowMobileMenu(false)} src={assets.icon} className='w-6' alt=''/></div>
        <ul className='flex flex-col items-center gap-2  mt-5 px-5 text-lg font-medium '>
          ...
          <a   onClick={() => setShowMobileMenu(false)}href='#Testimonial' className='px-4 py2 rounded-full inline-block'>Testimonial</a>
        </ul>
      </div>
    </div>
  )
